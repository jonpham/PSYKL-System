...
Production work is phase-gated: one DevTask at a time, stop at DevTask and Spec boundaries, wait for explicit approval. The gating rules live in [`docs/workflows/production-dev-workflow.md`](docs/workflows/production-dev-workflow.md). Experimental work is not phase-gated — see Workflow Routing below.

## Workflow Routing

Two lanes. Pick one before touching code, and say which lane you are in.

| Lane             | Use when                                                                     | Governed by                                                                                          | Artifacts                                                                                               |
| ---------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| **Experimental** | Prototyping a user-facing UI/UX idea whose value is unproven. UI layer only. | [`docs/workflows/experimental-feature-workflow.md`](docs/workflows/experimental-feature-workflow.md) | `docs/experiments/{slug}/` — feature card, one visual artifact, acceptance checks, implementation notes |
| **Production**   | Shipping a decided feature into the product.                                 | [`docs/workflows/production-dev-workflow.md`](docs/workflows/production-dev-workflow.md)             | Initiative → Spec → DevTask → feature doc                                                               |

- **Experimental work lives only in a client's `src/experiment/` tree** and is reachable at `/exp/{feature-slug}`. No schema, API, or shared-model changes; no edits to production modules beyond the sanctioned seam. An experiment ends discarded, paused, or promoted — promotion re-enters the Production lane with the experiment's artifacts as inputs.
- **The Experimental lane does not bypass this file.** Style ([`docs/STYLE.md`](docs/STYLE.md)), lint, typecheck, commit messages, and Git Conventions apply unchanged. Only the planning ceremony and the test-layer floor are reduced, and only as the experimental workflow document states.
- **Within the Experimental lane, apply rigor proportionally** — Fast Lane for small reversible UI changes, Standard Lane for normal user-facing features, High-Rigor Lane for risky or expensive-to-reverse features. The lane definitions live in the workflow document; do not restate them here.
...
- **Scope of the E2E mandate: production surfaces only.** Routes under `/exp/*` served from a client's `src/experiment/` tree are excluded. An experiment's test floor is one Storybook story exercising the primary acceptance check, plus Unit tests where logic warrants them — no E2E spec and no Integration test. Promoting an experiment into the Production lane restores the full mandate; the promotion DevTask writes the E2E spec.
...
| `docs/workflows/`                                  | Lane definitions — production development workflow and experimental feature workflow                              |
| `docs/experiments/{slug}/`                         | Experimental-lane artifacts: feature card, visual artifact, acceptance checks, implementation notes, screenshots  |
