...
| ---------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Experimental** | Prototyping a user-facing UI/UX idea whose value is unproven. UI layer only. | [`docs/workflows/experimental-feature-workflow.md`](docs/workflows/experimental-feature-workflow.md) | `docs/experiments/{slug}/` — experiment card, plus one `{iteration}/` subfolder per iteration holding feature card, one visual artifact, acceptance checks, implementation notes |
...
- **Experimental work lives only in a client's `src/experiment/` tree** and is reachable at `/exp/{experiment-slug}`. No schema, API, or shared-model changes; no edits to production modules beyond the sanctioned seam. An experiment may be iterated: the route and folder stay fixed while successive iterations add features to the same shell, each with its own artifact subfolder. An experiment — or a single iteration of one — ends discarded, paused, or promoted; promotion re-enters the Production lane with that iteration's artifacts as inputs, one iteration mapping to one Spec.
...
| `docs/experiments/{slug}/`                         | Experimental-lane artifacts: experiment card, plus `{iteration}/` subfolders (feature card, visual artifact, acceptance checks, implementation notes, screenshots) |
