...
export function CaptureRow({ onCancel, onCreate }: CaptureRowProps) {
  ...
  // the New Task button and are lost — "Book dentist" arrived as "k dentist".
  useLayoutEffect(() => {
  ...
  return (
    <li className="psykl-capture-row">
      <span aria-hidden="true" className="psykl-capture-row__bullet">
      ...
      <input
        ...
        className="psykl-capture-row__input"
        maxLength={200}
      ...
      />
      {errorMessage ? (
        <p className="psykl-capture-row__error" role="alert">
          {errorMessage}
        </p>
      ) : null}
    </li>
