...
**Initiative status:** 🟡 Active. Specs 1-4 are implemented and await review as a Pull Request stack; Spec 5 is two-thirds done (DevTask 11 is blocked on an operator decision); Spec 6 remains outlined in [`docs/specs/to-do-ui/`](specs/to-do-ui/).
...
**Active spec for execution:** N/A — `to-do-ui` Specs 1-4 are written out; their ten DevTask Pull Requests await review.
**Current execution:** `to-do-ui` Specs 1-4 complete and Spec 5 partly done, across a twelve-deep Pull Request stack — [#100](https://github.com/jonpham/PSYKL-System/pull/100) (merged), [#101](https://github.com/jonpham/PSYKL-System/pull/101), [#103](https://github.com/jonpham/PSYKL-System/pull/103), [#104](https://github.com/jonpham/PSYKL-System/pull/104), [#105](https://github.com/jonpham/PSYKL-System/pull/105), [#106](https://github.com/jonpham/PSYKL-System/pull/106), [#107](https://github.com/jonpham/PSYKL-System/pull/107), [#108](https://github.com/jonpham/PSYKL-System/pull/108), [#109](https://github.com/jonpham/PSYKL-System/pull/109), [#110](https://github.com/jonpham/PSYKL-System/pull/110), [#111](https://github.com/jonpham/PSYKL-System/pull/111), [#112](https://github.com/jonpham/PSYKL-System/pull/112).

**Next executable work:** `to-do-ui` Spec 5 DevTask 11 — stale writes in the Sync view — **blocked**: the Spec's three Data Model questions need operator answers before any implementation. Spec 6 follows.
**Branch:** `feat/to-do-ui-s5-dt12-recently-deleted-and-settings`, the tip of the stack.
**Known blockers:** None blocking. Two intermittent End-to-End failures are documented in the Spec 2 feature doc's Known Issues — a queued completion lost across a reload, and an occasional no-op list re-order. Both predate Spec 2 and are unmasked. The contrast question raised on 2026-09-21 is resolved — the app ships **Standard** (default) and **Increased** contrast as a device-local Settings choice, rather than choosing between Apple's conventional grays and WCAG AA. See [`docs/DESIGN.md`](DESIGN.md) → Contrast — Standard and Increased, and `to-do-ui` DESIGN.md → Decision 5.
**Workflow lanes:** Production (Initiative → Spec → DevTask) per [`docs/workflows/production-dev-workflow.md`](workflows/production-dev-workflow.md); Experimental (UI/UX prototyping at `/exp/{slug}`) per [`docs/workflows/experimental-feature-workflow.md`](workflows/experimental-feature-workflow.md). Routing table in [`AGENTS.md`](../AGENTS.md) → Workflow Routing. **Active experiment:** none. `apple-reminders-ux` is **accepted and promoted** — all three iterations (`sidebar-navigation`, `sync-status`, `apple-reminders-ui`) are absorbed by the `to-do-ui` initiative, which retires the experiment in its Spec 6.
...
| `to-do-ui`                    | Migrate the accepted `apple-reminders-ux` prototype into production | 🟡 Active — 6 Specs outlined, none started           | [`to-do-ui/`](initiatives/to-do-ui/)                   |
