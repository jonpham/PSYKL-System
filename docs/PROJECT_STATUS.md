...
**Active spec for execution:** N/A. The lightweight `target = production` change for [#126](https://github.com/jonpham/PSYKL-System/issues/126) — select and edit multiple list items — is complete and consolidated in its [feature doc](features/%5B20260923%5DGH126_to-do-ui-task-selection-and-batch-editing.md); its artifact folder was deleted at close-out. Earlier lightweight work: [#125](https://github.com/jonpham/PSYKL-System/issues/125) — PSYKL logo and app identity ([feature doc](features/%5B20260922%5DGH125_to-do-ui-psykl-logo-and-app-identity.md), merged as [PR #133](https://github.com/jonpham/PSYKL-System/pull/133)), [#123](https://github.com/jonpham/PSYKL-System/issues/123) ([feature doc](features/%5B20260922%5DGH123_to-do-ui-experiment-tools-toggle.md)), [#124](https://github.com/jonpham/PSYKL-System/issues/124) ([PR #131](https://github.com/jonpham/PSYKL-System/pull/131)), and [#122](https://github.com/jonpham/PSYKL-System/issues/122) ([PR #130](https://github.com/jonpham/PSYKL-System/pull/130)).
**Current execution:** None. The lightweight `target = production` change for [#127](https://github.com/jonpham/PSYKL-System/issues/127) — view, rename, and delete a single task from a details drawer — is implemented, operator-verified on an iPhone, and consolidated in its [feature doc](features/%5B20260923%5DGH127_to-do-ui-task-item-details-drawer.md); its artifact folder was deleted at close-out. Tapping a title reveals an **(i)** in the drag handle's column, which opens a drawer holding an editable title, the task's three timestamps, and a two-press delete; the inline title field is now an auto-growing `textarea`, so a tapped row no longer collapses a wrapped title and jumps. Both drawers gained the delete-list dialog's scrim and become centred modals past 768px. No schema or API change: the timestamps already ship on the `Task` payload and the delete reuses the existing soft-delete path. Green at every layer (401 unit, 54 Storybook component, 79 E2E). Earlier lightweight work on this surface: [#138](https://github.com/jonpham/PSYKL-System/issues/138) ([feature doc](features/%5B20260923%5DGH138_to-do-ui-bulk-edit-and-list-deletion-fixes.md)).

**Next executable work:** `to-do-ui` Spec 6 — retire the experiment and close out the initiative ([outline](specs/to-do-ui/20260921-Spec6-retire-experiment-and-close-out.md)). Opened by #126 and not yet scheduled: [#137](https://github.com/jonpham/PSYKL-System/issues/137) (a bulk-edit contract across both client paradigms) and [#136](https://github.com/jonpham/PSYKL-System/issues/136) (persist hand order as `Task.position`, mirroring List ordering's fractional index). Also unscheduled, surfaced by #138: stories that render `<App />` do not import `styles/tokens.css` (only `main.tsx` does), so any such story runs with no design tokens — fixed in the files #138 and #127 touched, still open elsewhere.
**Branch:** `feat/127-list-item-details-drawer` ([PR #140](https://github.com/jonpham/PSYKL-System/pull/140), open, awaiting operator merge).
