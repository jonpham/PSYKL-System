...
**Active spec for execution:** M2 Spec 2 — PWA IndexedDB store + `useSyncExternalStore` ([`20260610-S2-pwa-indexeddb-store.md`](specs/m2-pwa-crud-offline/20260610-S2-pwa-indexeddb-store.md)).
**Next executable step:** Complete M2 Spec 2 DevTask M2-6: add `useTasks()` and refactor UI reads to IndexedDB.
**Active skill:** `superpowers:executing-plans` executing M2 Spec 2 one DevTask at a time.
**Branch:** `feat/m2-s2-dt5-idb-store`, branched from `spec/m2-s2-pwa-indexeddb-store`.
**Known blockers:** None.
...
| M2-2 | PWA IndexedDB store + useSyncExternalStore    | [`20260610-S2-pwa-indexeddb-store.md`](specs/m2-pwa-crud-offline/20260610-S2-pwa-indexeddb-store.md)                                            | M2-5 complete; M2-6 next |
