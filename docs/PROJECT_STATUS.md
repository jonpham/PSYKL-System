...
**Current execution:** `to-do-ui` lightweight `target = production` fix for [#138](https://github.com/jonpham/PSYKL-System/issues/138) — the three UX defects #126 left behind, plus cascade-aware Recently Deleted. Artifacts in [`docs/specs/to-do-ui/20260923-bulk-edit-remaining-ux-fixes/`](specs/to-do-ui/20260923-bulk-edit-remaining-ux-fixes/). Implemented in two slices on `bug/138-remaining-issues-list-bulk-edit`: batch completion is a toggle and the armed trash is a ring; Delete List opens a **With Items / Just the List / Cancel** dialog and a list restores with the tasks deleted alongside it. No schema or API change. Green at every layer (387 unit, 50 Storybook component, 71 E2E). Awaiting operator review, then a PR.

**Next executable work:** Open and merge the [#138](https://github.com/jonpham/PSYKL-System/issues/138) PR with operator approval; then `to-do-ui` Spec 6 — retire the experiment and close out the initiative ([outline](specs/to-do-ui/20260921-Spec6-retire-experiment-and-close-out.md)). Opened by #126 and not yet scheduled: [#137](https://github.com/jonpham/PSYKL-System/issues/137) (a bulk-edit contract across both client paradigms) and [#136](https://github.com/jonpham/PSYKL-System/issues/136) (persist hand order as `Task.position`, mirroring List ordering's fractional index).
