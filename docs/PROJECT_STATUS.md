...
**Current execution:** None. The lightweight `target = production` fix for [#138](https://github.com/jonpham/PSYKL-System/issues/138) — the three UX defects #126 left behind, plus cascade-aware Recently Deleted — is implemented, operator-verified on an iPhone, and consolidated in its [feature doc](features/%5B20260923%5DGH138_to-do-ui-bulk-edit-and-list-deletion-fixes.md); its artifact folder was deleted at close-out. Batch completion is a toggle, the armed batch trash is a ring rather than a fill, `Delete List` opens a **With Items / Just the List / Cancel** dialog, and a restored list brings the tasks deleted alongside it back with it. No schema or API change: both cascades are client-orchestrated batches over existing calls, and which tasks went with a list is derived from a shared `deleted_at`. Green at every layer (387 unit, 50 Storybook component, 71 E2E).

**Next executable work:** `to-do-ui` Spec 6 — retire the experiment and close out the initiative ([outline](specs/to-do-ui/20260921-Spec6-retire-experiment-and-close-out.md)). Opened by #126 and not yet scheduled: [#137](https://github.com/jonpham/PSYKL-System/issues/137) (a bulk-edit contract across both client paradigms) and [#136](https://github.com/jonpham/PSYKL-System/issues/136) (persist hand order as `Task.position`, mirroring List ordering's fractional index). Also unscheduled, surfaced by #138: stories that render `<App />` do not import `styles/tokens.css` (only `main.tsx` does), so any such story runs with no design tokens — fixed in the three files #138 touched, still open elsewhere.
**Branch:** `bug/138-remaining-issues-list-bulk-edit`.
