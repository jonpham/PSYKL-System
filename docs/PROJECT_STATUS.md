...
**Active skill:** `/plan-design-review` — complete. `/plan-eng-review` is next.
**Branch:** `feat/plan-psykl-loop`.
**Known blockers:** None. Next action: run `/plan-eng-review` against [`UX.md`](initiatives/todo-experience/UX.md) and [`DESIGN.md`](initiatives/todo-experience/DESIGN.md) to lock the four decisions the design review deliberately left to engineering: section representation, fractional-position representation, sync-queue migration vs drain, and where device-local preferences (theme choice, section collapse state) are stored outside the sync queue.
