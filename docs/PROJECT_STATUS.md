...
**Active spec for execution:** `todo-experience` Spec 2 — Recently Deleted + offline posture ([`20260818-Spec2-recently-deleted-and-offline-posture.md`](specs/todo-experience/20260818-Spec2-recently-deleted-and-offline-posture.md)). DevTasks 7/12 (Restore endpoints + `GET /deleted`), 8/12 (30-day purge job), and 9/12 (Orphan sweep) merged via PRs [#76](https://github.com/jonpham/PSYKL-System/pull/76), [#77](https://github.com/jonpham/PSYKL-System/pull/77), [#78](https://github.com/jonpham/PSYKL-System/pull/78). DevTask 10/12 (Restore sync-queue plumbing) implemented, PR [#79](https://github.com/jonpham/PSYKL-System/pull/79) open against the Spec branch, awaiting review/merge. DevTasks 7-8 were split from DESIGN.md's combined "restore + purge" DevTask per the AGENTS.md trilemma rule (11 files > ≤10 ceiling); the former DevTask 10 ("Recently Deleted screen + restore UI") was further split into DevTask 10 (plumbing, this PR) and a new DevTask 11 (screen, depends on DevTask 10) — combined scope was 12+ files. Former DevTask 11 (Offline pressure) renumbers to DevTask 12; devtasks_total is now 6. See spec doc's "Decisions made during spec drafting" for both splits, and the Entry-point scope decision (DevTask 11 uses a temporary button, not UX.md's unbuilt `⋯` overflow menu).
**Next executable spec:** N/A — mid-Spec 2. After DevTask 10 merges into the Spec branch, DevTask 11 (Recently Deleted screen) starts next.
**Branch:** `spec/todo-experience-s2-recently-deleted-and-offline-posture` (Spec integration, draft PR [#75](https://github.com/jonpham/PSYKL-System/pull/75)); active DevTask branch `feat/todo-experience-s2-dt10-restore-plumbing`.
**Known blockers:** None. Next action: review + merge PR #79, then branch DevTask 11 off the updated Spec branch.
