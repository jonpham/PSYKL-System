...
**Active initiative:** `psykl-loop` — PSYKL execution + retrospectives on the PWA. Sketched at [`docs/initiatives/psykl-loop/MILESTONE.md`](initiatives/psykl-loop/MILESTONE.md); design doc not yet drafted.
**Initiative status:** 🟡 `psykl-loop` is active but sketched, not designed — the next action is an `/office-hours` design pass. The preceding initiative, M2 — PWA CRUD + offline-first, is 🟢 complete and merged to `main` (PR [#66](https://github.com/jonpham/PSYKL-System/pull/66), commit `62e4105`). Specs 1-2 shipped to `main` in `v0.2.0`; Specs 3-4 shipped via merged Spec PR [#56](https://github.com/jonpham/PSYKL-System/pull/56); Specs 5-6 and initiative close-out shipped via PR #66. Durable records live in [`docs/features/`](features/), [`docs/ARCHITECTURE.md`](ARCHITECTURE.md), [`docs/STACK.md`](STACK.md), [`CHANGELOG.md`](../CHANGELOG.md), and [`docs/retrospectives/2026-07-29-m2-pwa-crud-offline.md`](retrospectives/2026-07-29-m2-pwa-crud-offline.md).
...
**Next executable spec:** N/A — no specs exist for `psykl-loop` yet. `superpowers:writing-plans` runs after the design doc is APPROVED.
**Active skill:** N/A.
**Branch:** `main`.
**Known blockers:** None. Next action: draft `docs/initiatives/psykl-loop/DESIGN.md` via `/office-hours` on a `feat/plan-psykl-loop` branch. The `cycle` entity shape (it crosses `service-task`, the IndexedDB store, and the sync queue) and the coaching-rule model are the two decisions worth the design pass.
...
**Roadmap re-cut (2026-08-12).** Apple-native client work is **deferred** in favour of growing the PWA into a semi-mature product first. The product-discovery scope that used to be bundled with the native clients (PSYKL execution, boundary coaching, daily plan, retrospectives) moved to the new `psykl-loop` milestone, where it ships on the PWA — those features never needed a native client, and building them first produces the dogfooding signal the native milestone wanted anyway. At the same time, **milestones past the shipped work are now identified by a short purpose tag rather than an ordinal number**, because the remaining roadmap is a set of candidates, not a sequence. Shipped milestones keep `M1` / `M2` (those tokens are baked into ADR IDs, feature-doc filenames, and `CHANGELOG.md`). See [`docs/PRODUCT.md`](PRODUCT.md) → Milestone Roadmap.
...
1. **Read in this order:**
   ...
   - [`docs/initiatives/psykl-loop/MILESTONE.md`](initiatives/psykl-loop/MILESTONE.md) — the active initiative; draft its design doc from `main`
...
| `psykl-loop`                  | PSYKL execution, boundary coaching, daily plan, retrospectives | 🟡 Active — sketched        | [`psykl-loop/`](initiatives/psykl-loop/)               |
...
| Configurable term-map / UI theme architecture (default celestial: PSYKL/Earth/Moon/HelioArc/Sun)                                                                                                                                                                                                           | `/office-hours`                           | Unassigned          |
