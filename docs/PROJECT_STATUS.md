...
**Initiative status:** 🟡 Active. Six Specs outlined in [`docs/specs/to-do-ui/`](specs/to-do-ui/), each expanded via `superpowers:writing-plans` when it starts. No Spec has begun.
...
**Next executable spec:** `to-do-ui` Spec 1 — Shell, Navigation, and List Management ([outline](specs/to-do-ui/20260921-Spec1-shell-navigation-and-lists.md)). Start with `superpowers:brainstorming`, then `writing-plans` to expand the outline, then `executing-plans`.
...
**Known blockers:** One operator decision, needed before `to-do-ui` Spec 1 writes the token sheet — **the contrast exception**. Adopting Apple's system colors imports Apple's contrast behavior: `--text-secondary` measures 3.3:1 and `--text-tertiary` (the unchecked checkbox stroke) 1.7:1, both below WCAG AA. Three options with a recommendation are in [`docs/DESIGN.md`](DESIGN.md) → Contrast — an open decision.
