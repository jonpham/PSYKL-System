...
**Initiative status:** 🟡 Active. Six Specs outlined in [`docs/specs/to-do-ui/`](specs/to-do-ui/); Spec 1 implementation is in progress.
...
**Active spec for execution:** N/A — the `to-do-ui` planning artifacts are landing; no Spec has started.
**Current execution:** `to-do-ui` Spec 1/6, DevTask 1/4 — production token sheet, URL-backed destinations, and glyphs on `feat/to-do-ui-s1-dt1-foundation`.

**Next executable spec:** `to-do-ui` Spec 1 — Shell, Navigation, and List Management ([implementation spec](specs/to-do-ui/20260921-Spec1-shell-navigation-and-lists.md)). Continue DevTask 1 with `superpowers:executing-plans`.
**Branch:** `feat/plan-to-do-ui` — initiative planning branch, docs only, merges to `main` independently of any Spec branch. No active Spec branch. Release `v0.4.1` ("Polished UI Prototype") cut from `main` and deployed to `robin`.
