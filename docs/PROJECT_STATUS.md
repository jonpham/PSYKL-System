...
**Active spec for execution:** `todo-experience` Spec 2 — Recently Deleted + offline posture ([`20260818-Spec2-recently-deleted-and-offline-posture.md`](specs/todo-experience/20260818-Spec2-recently-deleted-and-offline-posture.md)). DevTask 7/11 (Restore endpoints + `GET /deleted`) merged via PR [#76](https://github.com/jonpham/PSYKL-System/pull/76). DevTask 8/11 (30-day purge job) merged via PR [#77](https://github.com/jonpham/PSYKL-System/pull/77). DevTask 9/11 (Orphan sweep) implemented, PR [#78](https://github.com/jonpham/PSYKL-System/pull/78) rebased onto the Spec branch (both #77 and #78 originally stacked on #76; #78 depended on DevTask 7, not 8, so it rebased cleanly once #76 merged), awaiting review/merge. DevTasks 7-8 were split from DESIGN.md's combined "restore + purge" DevTask per the AGENTS.md trilemma rule (11 files > ≤10 ceiling) — DevTasks renumbered 7-11, see spec doc's "Decisions made during spec drafting". DevTask 9's healing logic was further split into its own file (`task-orphan-sweep.ts`) mid-DevTask to satisfy the `max-lines` ESLint rule.
**Next executable spec:** N/A — mid-Spec 2. After DevTask 9 merges into the Spec branch, DevTask 10 (Recently Deleted UI) starts next.
**Branch:** `spec/todo-experience-s2-recently-deleted-and-offline-posture` (Spec integration, draft PR [#75](https://github.com/jonpham/PSYKL-System/pull/75)); active DevTask branch `feat/todo-experience-s2-dt9-orphan-sweep`.
**Known blockers:** None. Next action: review + merge PR #78, then branch DevTask 10 off the updated Spec branch.
