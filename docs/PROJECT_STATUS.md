...
**Active spec for execution:** Lightweight `target = production` change for [#126](https://github.com/jonpham/PSYKL-System/issues/126) — select and edit multiple list items. Planning artifacts at [`docs/specs/to-do-ui/20260922-multiple-list-item-edit-selection/`](specs/to-do-ui/20260922-multiple-list-item-edit-selection/), awaiting operator review. Previously: the lightweight production change for [#123](https://github.com/jonpham/PSYKL-System/issues/123) — switching between production and experiment experiences from the 🧪 pill — is complete and consolidated in its [feature doc](features/%5B20260922%5DGH123_to-do-ui-experiment-tools-toggle.md); its artifact folder was deleted at close-out. The in-app PWA update for [#124](https://github.com/jonpham/PSYKL-System/issues/124) merged as [PR #131](https://github.com/jonpham/PSYKL-System/pull/131), and the parity change for [#122](https://github.com/jonpham/PSYKL-System/issues/122) as [PR #130](https://github.com/jonpham/PSYKL-System/pull/130).
**Current execution:** #126 planning only — the four lightweight artifacts are written and implementation is paused for operator review. One backend decision is open: manual task re-ordering needs a `position` field on `Task` (see that folder's `implementation-notes.md`). Previously: #123 is implemented, operator-approved on the local stack, and covered at every test layer (unit, Storybook component, E2E); [PR #132](https://github.com/jonpham/PSYKL-System/pull/132) carries the whole change including its docs, so merging it is the last step. `to-do-ui` Specs 1-5 are merged and released as v0.5.0.

**Next executable work:** Operator review of the #126 artifacts and the `Task.position` decision, then its first implementation slice; then `to-do-ui` Spec 6 — retire the experiment and close out the initiative ([outline](specs/to-do-ui/20260921-Spec6-retire-experiment-and-close-out.md)).
