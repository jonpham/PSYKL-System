...
- `components/web_client/src/components/VersionFooter/VersionFooter.tsx` + `version-footer.css` — rewritten presentation per the visual artifact; API commit demoted to a detail row.
- `components/web_client/src/components/SettingsView/SettingsView.tsx` — unchanged (already renders `<VersionFooter />` under About).

Five production source files — inside the ≤10 per-PR limit.
...
- **During implementation:** TDD with Unit tests only — `version.ts` (available-version parse, non-JSON rejection), `useAppUpdate` (up-to-date / update-available / failed / timeout-fallback transitions), `VersionFooter` (the five states render per the storyboard).
- **After approval, in the same PR before merge:** add the E2E spec (`e2e/settings.e2e.spec.ts`, titles taken from the acceptance checks) and update `VersionFooter.stories.tsx` / `SettingsView.stories.tsx` to the new UX. Static analysis (lint, format, typecheck) is **not** reduced at any point.
...
- Keep the component name `VersionFooter`, or rename to `AppVersion` now that it is not a footer? (Rename adds a file move plus test/story renames; default is keep.)
- Show `builtAt` as a human date next to the commit, or commit only? (Default: commit only, matching today.)
