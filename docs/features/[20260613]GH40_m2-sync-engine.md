...
prs:
  ...
  - pending: feat/m2-s3-dt8-sync-page-triggers
...
3. Replay uses an IndexedDB `sync_meta.replay_lock` row with a 30-second stale timeout so page and future Service Worker contexts do not drain the queue concurrently.
...
5. `TaskCreateForm` now writes an optimistic local Task, enqueues the create mutation, and triggers replay instead of calling `POST /tasks` directly.
...
- Constituent DevTask PRs: [#57](https://github.com/jonpham/PSYKL-System/pull/57) and `feat/m2-s3-dt8-sync-page-triggers` (pending PR at close-out commit time).
...
- **M2-8** added page replay triggers, optimistic create queueing, pending-row affordances, and the permanent-failure toast surface.
...
| 2026-06-13 | pending                                                | M2-8: page triggers, optimistic create queueing, pending UI, toast.     |
