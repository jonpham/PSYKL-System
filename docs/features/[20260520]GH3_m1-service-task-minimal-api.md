...
- **ADR-M1-004:** NestJS keeps REST, GraphQL, and gRPC open behind a shared service layer.
- **ADR-M1-005:** Zod schemas are the source of truth for runtime validation, TypeScript types, and OpenAPI output.
- **ADR-M1-006:** pglite keeps M1 local-first while preserving PostgreSQL semantics for later milestones.
- **ADR-M1-007:** UUID v7 improves insertion locality and supports future offline-first client generation.
- **ADR-M1-008:** `UserIdGuard` is the M1/M2 authorization boundary; `user_id` ownership is the access model.
- **ADR-M1-009:** Static analysis is centralized at the repo root through ESLint flat config and Prettier, with package scripts invoking the shared configuration.
