...
1. **Split lower-layer CI workflow.** `.github/workflows/ci.yml` runs four required checks on Pull Requests to `main` and `spec/**`: `static-checking`, `unit-tests`, `integration-tests`, and `component-tests`.
...
7. **Branch-protection instructions.** README and the close-out docs record the required status checks for `main`: `CI / static-checking`, `CI / unit-tests`, `CI / integration-tests`, `CI / component-tests`, and `CI E2E / e2e`.
...
- **CI decomposition:** Lower test layers are split into separate jobs so GitHub branch protection can require each layer independently. This also makes failures easier to scan in Pull Request status.
...
- **Manual branch protection:** Branch protection is intentionally a repository setting, not code. Steps 13-14 remain post-merge manual verification because the required checks must exist on `main` before they can be selected.
...
11. Open PR #29 and confirm the five expected required-check candidates run on the pushed branch.
12. After PR #29 merges and workflows have run on `main`, configure branch protection to require `CI / static-checking`, `CI / unit-tests`, `CI / integration-tests`, `CI / component-tests`, and `CI E2E / e2e`.
...
- **#17** GitHub branch protection on `main` is configured in repository settings, not code.
...
- **ADR-M1-022:** GitHub Actions is the M1 CI gate. It runs on Pull Requests to `main` and Spec integration branches so long-lived Spec PRs receive the same feedback as final merges.
- **ADR-M1-023:** Static Analysis is an upstream required check. Unit, Integration, and Component jobs declare `needs: static-checking` so lint, formatting, and type errors stop deeper lower-layer work.
...
| Date       | PR                                                     | Summary                                                                                                                                                                |
| ---------- | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-29 | [#29](https://github.com/jonpham/PSYKL-System/pull/29) | Spec integration: split CI checks, root `verify:*` command surface, Playwright E2E workspace, reusable E2E helper scripts, branch-protection docs, and close-out docs. |
