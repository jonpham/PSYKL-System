...
- **Comments describe current behavior, never history and never transient workflow.** Two bans, both absolute:
  - **No workflow identifiers.** Initiative tags, Spec numbers, DevTask numbers, branch names, and phase labels are transient development bookkeeping; they are meaningless to a reader six months out and stale within the week. Durable external keys — a GitHub Issue or Pull Request number — are the only acceptable trackable reference.
  - **No notes addressed to a future agent or contributor.** "Un-skip this in DevTask 4", "delete this comment after implementation", "the next Spec finishes this" — none of these belong in code. A skipped test, a staged migration, or a deliberately incomplete surface is planned work, and planned work lives in the plan documents under `docs/specs/`. An agent learns what to activate from its Spec doc's activation table, not from a breadcrumb in a test file.
  - Corollary: **historical framing is not behavior.** "Rewritten for…", "this used to be a modal", "moved off the old sheet" — state what the code does now; git carries what it did before.
