...
| ------------------- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Clear               | `Groceries` with a green `↻ Sync` control; no sync banner above the Task form                            | No queued or permanently failed changes              |
| Attention           | `Groceries` with a yellow `↻ Sync · 3` control; the Task form remains directly below the header          | One or more changes are queued or permanently failed |
...
- Green means both counts are zero; yellow means either count is non-zero. Color is reinforced by text, count, and accessible name.
